...
def entropy(class_y):
    ...
    prob_zero = float(zeros)/len(class_y)
    prob_one = float(ones)/len(class_y)

    if prob_one == 1 or prob_zero == 1:
        entropy = 0

    else:
        entropy = stats.entropy(prob_zero, base=2) + stats.entropy(prob_one, base=2)
...
def information_gain(previous_y, current_y):
    ...
    for j in current_y:
        ...
        info_gain -= entropy(j)*len(j)/len(previous_y)
